...
export class DodaniePozycjiStolikaWUI1768229504246 implements MigrationInterface {
    name = 'DodaniePozycjiStolikaWUI1768229504246'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`stolik\` ADD \`pozycjaX_UI\` int NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`stolik\` ADD \`pozycjaY_UI\` int NOT NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE UNIQUE INDEX \`IDX_9aa1667dcc5d2fa04faf959f82\` ON \`restauracja_obraz\` (\`restauracjaRestauracjaId\`, \`czy_glowne\`)`);
        await queryRunner.query(`ALTER TABLE \`restauracja_obraz\` ADD CONSTRAINT \`FK_efb1a07fe96e93bbdfe25e5b28a\` FOREIGN KEY (\`restauracjaRestauracjaId\`) REFERENCES \`restauracja\`(\`restauracja_id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`stolik\` DROP COLUMN \`pozycjaY_UI\``);
        await queryRunner.query(`ALTER TABLE \`stolik\` DROP COLUMN \`pozycjaX_UI\``);
    }
