...
export class FixRestauracjaObraz1765729138516 implements MigrationInterface {
    name = 'FixRestauracjaObraz1765729138516'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`restauracja\` DROP COLUMN \`zdjecie\``);
        await queryRunner.query(`ALTER TABLE \`restauracja_obraz\` ADD \`czy_glowne\` tinyint NOT NULL DEFAULT 0`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`restauracja_obraz\` ADD CONSTRAINT \`FK_efb1a07fe96e93bbdfe25e5b28a\` FOREIGN KEY (\`restauracjaRestauracjaId\`) REFERENCES \`restauracja\`(\`restauracja_id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`restauracja_obraz\` DROP COLUMN \`czy_glowne\``);
        await queryRunner.query(`ALTER TABLE \`restauracja\` ADD \`zdjecie\` mediumblob NULL DEFAULT 'NULL'`);
    }
